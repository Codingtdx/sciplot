...
- Plot keeps its Source rail visible by default and lets it auto-compact when space is tight. The rail owns source state, the plot object list, and recommended templates; it should not expose a manual source hide button.
- Plot tools are creation/selection entry points, not drag-first editors. The left Objects list selects figure objects, and the right selection inspector is the default place for exact scientific parameters.
