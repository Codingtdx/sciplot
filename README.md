...
The Launcher is the supported app entry surface. It only opens or focuses modules; real import/open/bind actions live in the destination module window toolbar or menu. Pixelmator-style presentation is treated as interaction grammar only; do not add fake drawing tools.
...
- Module primary actions live in the native window toolbar/menu, visually anchored at the upper right: `Import/Open`, `Export`, `New Project`, `Launcher`, `Help`, and `Inspector` must not drift back into left selection panels or inspector bodies. Plot also exposes a toolbar `Data Workbook` icon.
- Appearance is app-wide and lives under `View > Appearance`: `Follow System`, `Light`, and `Dark`. Light mode is intentionally near-white and Codex-like rather than cool gray; dark mode keeps the current pro workspace feel.
- Custom Liquid Glass surfaces must use the shared shaped helpers in `StateViews.swift` (`proGlassPanel`, `proGlassRail`, `proGlassRow`) so fills, clipping, outlines, and glass sampling use the same rounded shape. Do not restore raw rectangular `.background(theme.panelFill)` behind rounded panels.
- Frontend layout and interaction handoff lives in `docs/macos-frontend-design.md`. The macOS app icon source is `docs/assets/sciplot-god-app-icon.svg`; generated icon assets live in `app/macos/Assets.xcassets/AppIcon.appiconset`. The icon is an abstract native-style glass/data-flow mark, not a literal chart page.
...
- Plot exposes a toolbar `Data Workbook` utility affordance that opens a read-only sheet with `Source Data`, `Transformed`, `Variables`, and `Fit` tabs, defaulting to `Source Data` from the toolbar. `Transformed` previews backend-applied typed data transforms, `Variables` shows durable scalar/expression variables, and `Fit` supports `linear`, `polynomial_2`, `polynomial_3`, `exponential`, `logarithmic`, `power_law`, `gaussian`, `logistic`, and bounded `custom_function` models. The default Fit inspector flow uses large model cards and compact equation/stat summaries; Data Workbook remains the detailed residual/table view.
- Plot uses a dark pro workspace with a Pixelmator-style four-zone layout: left data/plot-type panel, central white figure/page preview, right glass adjustment inspector, and far-right adjustment category rail. The left panel only keeps a compact sheet picker, five recommended plot-type thumbnail cards from `templateGalleryItems`, and a `More` sheet backed by `plotTypeItems`; it must not show file names, import buttons, Data Workbook tab rows, or become a layer/object list again.
- Plot live preview prefers a PNG bitmap payload returned by `/render-preview`, while the same response keeps PDF base64 as the authoritative exact preview/export fallback and may include `interaction_metadata` for canvas hit mapping. Do not duplicate plotting semantics in Swift; the backend renderer remains the preview/export truth source.
- The far-right Plot rail is not a creation tools strip. It switches the right inspector between `Figure`, `Axes`, `Legend`, `Guides`, `Fit`, `Functions`, `Annotations`, and `Advanced Axes`; category buttons do not open popovers. `Legend` exposes legend ordering directly instead of hiding its only common action behind `Advanced`. Guides and annotations are canvas-first: inspector mode cards choose what the next click/drag will place, `InteractivePlotOverlay` writes the existing typed payloads, and the backend redraws the figure. Function layers remain inspector-first because expression sampling is backend-owned. `Data Cursor` stays out until preview hit-testing metadata supports real point selection.
- Plot import/open always belongs to the Plot window. If replacing an existing Plot source requires confirmation, that confirmation is hosted by Plot itself. `New Project` is the explicit path that clears Plot state and returns to Launcher.
- Plot adjustment inspector `Fit` exposes fit overlay controls only for curve-like templates (`curve`, `point_line`, `scatter`). Model selection is a card grid with UI-only abstract glyphs; overlay rendering, inspector summaries, and `Data Workbook -> Fit` must stay on the same shared backend fit helper.
...
- Plot Guide placement creates typed `Reference Guides` as exact-input scientific objects (`line` + `region`). Multiple guides can be layered, each guide can target `x`, `primary y`, or `secondary y`, and the whole stack flows through preview/export and `.sciplotgod` project persistence without changing frozen `nature` metrics.
...
- Plot Text and Shape placement creates typed annotations directly from the preview canvas, then hands editing to the selection inspector. Text notes/callouts and shape annotations (`rectangle`, `ellipse`, `bracket`) travel through the same preview/export/project payload path, reuse render metadata where available, and stay an opt-in refinement layer under the recommendation-first workflow instead of becoming a second free-form authoring model.
...
- Data Studio follows the same Pro workspace grammar: left side for `Workbook Groups` plus figure family/template choice, center for compare/figure preview and focused-group state, and right side for scientific figure adjustments, `Open in Plot`, `Analysis`, and exported-output follow-ups. The inspector must not duplicate the left-side template picker.
...
- Data Studio exposes an `Analysis` utility sheet with `Focused Workbook` and `Current Figure` scopes. `Source Data` shows paged workbook rows; `Fit` reuses the shared Plot card-based fit model surface and compact result summary. Current-figure fitting is limited to curve-like templates (`curve`, `point_line`, `scatter`).
...
- Composer keeps its real panel library on the left with `All / Graphs / Assets` filtering, canvas in the center, and a right inspector organized as `Selection / Placement / Panel / Actions / Preview`. Code Console keeps bound context plus sheet choice on the left, prompt/code/output in the center, and context/runner/output handoff inspector on the right; source open/reveal actions belong in inspector `Advanced` or toolbar utility space rather than the left panel.
...
- Inspector keeps high-frequency controls visible. `DisclosureGroup("Advanced")` is reserved for low-frequency, diagnostic, output follow-up, or genuinely advanced parameters, not for the only meaningful action in a category.
