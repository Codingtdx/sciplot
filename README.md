...
- Plot's inner left rail is templates-only. Source, sheet, and Data Workbook controls stay out of that rail so global import/export, data utility work, and template choice remain visually distinct.
- Plot tools are creation/selection entry points, not drag-first editors. The right selection inspector is the default place for exact scientific parameters.
