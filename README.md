...
- Data Studio follows the same Pro workspace grammar at framework level: left side for `Workbook Groups` plus figure family/template choice, center for compare/figure preview and focused-group state, and right side for figure/action inspector. This is a layout unification only, not a new fake tool surface.
...
- Composer keeps its real asset library on the left, canvas in the center, and selection/edit/preview inspector on the right. Code Console keeps bound context on the left, prompt/code/output in the center, and context/runner/output handoff inspector on the right; source open/reveal actions belong in inspector `Advanced` or toolbar utility space rather than the left panel.
