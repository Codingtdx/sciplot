...
- Plot exposes a `Data` utility affordance that opens `Data Workbook`, a read-only sheet with `Source Data` and `Fit` tabs. `Fit` supports `linear`, `polynomial_2`, and `polynomial_3`, reports the fitted equation/statistics plus per-point residual rows, and shares its selected model with Plot fit overlays.
