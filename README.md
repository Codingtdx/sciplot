...
- Module primary actions live in the native window toolbar/menu, visually anchored at the upper right: `Import/Open`, `Export`, `Launcher`, `Help`, and `Inspector` must not drift back into left selection panels or inspector bodies. Plot also exposes a toolbar `Data Workbook` icon.
...
- Frontend layout and interaction handoff lives in `docs/macos-frontend-design.md`. The macOS app icon source is `docs/assets/sciplot-god-app-icon.svg`; generated icon assets live in `app/macos/Assets.xcassets/AppIcon.appiconset`.
