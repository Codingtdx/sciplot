...
- Plot import/open always belongs to the Plot window. If replacing an existing Plot source requires confirmation, that confirmation is hosted by Plot itself, including when the action started from Launcher. `New Project` is the explicit path that clears Plot state and returns to Launcher.
