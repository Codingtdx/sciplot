...
def create_render_router(*, dep_provider: Callable[[], object] | None = None) -> APIRouter:
            ...
            return SourceTablePreviewResponse.model_validate(
                {
                    ...
                    "encoding": preview.encoding,
                    "delimiter": preview.delimiter,
                    "data_containers": _source_table_data_containers(
                        preview,
    ...
    def fit_analysis(request: FitAnalysisRequest) -> FitAnalysisResponse:
        try:
            ...
            fit_container = {
                "id": f"fit-result:{selected_series.series_id}",
                "kind": "fit_result",
                "label": f"{selected_series.series_label} fit result",
                "status": "enabled",
                "readonly": True,
                "row_count": len(selected_series.derived_rows),
                "column_count": 4,
                "columns": [
                    {"id": "fit-x", "name": selected_series.x_label or "x", "index": 0, "role_hints": ["x"]},
                    {"id": "fit-y", "name": selected_series.y_label or "y", "index": 1, "role_hints": ["y"]},
