...
extension ComposerSession {
    ...
    func mergeSelectedCells() {
        guard let selection = selectedCellSelection, mergeSelectedCellsAvailability.isEnabled else {
            return
        }

        let previous = project
        var candidate = project
        let regionID = nextAvailableID(prefix: "region", existing: candidate.regions.map(\.id))
    ...
    func canPlace(panelID: String, in target: ComposerPlacementTarget) -> Bool {
        ...
        if let occupantID {
            guard let occupant = panelByID(occupantID),
                  !occupant.locked,
                  let sourceTarget,
                  isStructurallyCompatible(panel: occupant, with: sourceTarget, in: project)
            else {
                return false
            }
    ...
    func regionCovering(cell: ComposerGridCell) -> ComposerRegionPayload? {
        project.regions.first { region in
            cells(for: region).contains(cell)
        }
    }

    func panelForRegion(_ regionID: String) -> ComposerPanelPayload? {
        panelByID(regionOccupants(regionID: regionID).first?.id)
    }
    ...
    func panelsAssigned(to regionID: String) -> [ComposerPanelPayload] {
        regionOccupants(regionID: regionID)
    }

    func regionSummary(_ region: ComposerRegionPayload) -> String {
        let origin = ComposerGridCell(col: region.col, row: region.row)
        return "Region \(cellDisplayLabel(origin)) • \(region.colSpan)×\(region.rowSpan)"
    }

    func cellDisplayLabel(_ cell: ComposerGridCell) -> String {
        let columnLabel = String(UnicodeScalar(65 + cell.col) ?? UnicodeScalar(65))
        return "\(columnLabel)\(cell.row + 1)"
    }

    func rectangularSelection(from cells: Set<ComposerGridCell>) -> ComposerCellSelection? {
    ...
    func rectangularCells(from start: ComposerGridCell, to end: ComposerGridCell) -> [ComposerGridCell] {
    ...
    func panelByID(_ panelID: String?) -> ComposerPanelPayload? {
    ...
    func panelByID(_ panelID: String?, in project: ComposerRequestPayload) -> ComposerPanelPayload? {
        guard let panelID else {
            return nil
        }
        return project.panels.first { $0.id == panelID }
    ...
    func regionByID(_ regionID: String) -> ComposerRegionPayload? {
        project.regions.first { $0.id == regionID }
    }

    func regionByID(_ regionID: String, in project: ComposerRequestPayload) -> ComposerRegionPayload? {
        project.regions.first { $0.id == regionID }
    }

    func regionRectMm(for region: ComposerRegionPayload, in project: ComposerRequestPayload) -> CGRect {
        CGRect(
    ...
    func cellRectMm(for cell: ComposerGridCell, in project: ComposerRequestPayload) -> CGRect {
        CGRect(
    ...
    func rectMm(for selection: ComposerCellSelection, in project: ComposerRequestPayload) -> CGRect {
        CGRect(
    ...
    func cellMatchingPanel(_ panel: ComposerPanelPayload, in project: ComposerRequestPayload) -> ComposerGridCell? {
        let tolerance = 0.25
    ...
    func placementTarget(for panel: ComposerPanelPayload) -> ComposerPlacementTarget? {
        placementTarget(for: panel, in: project)
    }

    func placementTarget(forPanelID panelID: String, in project: ComposerRequestPayload) -> ComposerPlacementTarget? {
    ...
    func placementTarget(for panel: ComposerPanelPayload, in project: ComposerRequestPayload) -> ComposerPlacementTarget? {
    ...
    func cells(for region: ComposerRegionPayload) -> [ComposerGridCell] {
    ...
    func cells(for target: ComposerPlacementTarget, in project: ComposerRequestPayload) -> [ComposerGridCell] {
        switch target {
        case let .cell(cell):
            return [cell]
        case let .freeRegion(regionID):
    ...
    func freeRegions(in project: ComposerRequestPayload) -> [ComposerRegionPayload] {
        project.regions
            .filter { $0.kind == "free" }
            .sorted { lhs, rhs in
    ...
    func structuralBlockedCells(in project: ComposerRequestPayload) -> Set<ComposerGridCell> {
        Set(freeRegions(in: project).flatMap(cells(for:)))
    }

    func panelOccupiedCells(panelID: String? = nil, in project: ComposerRequestPayload, excluding excludedPanelIDs: Set<String>) -> Set<ComposerGridCell> {
    ...
    func regionOccupants(regionID: String, in project: ComposerRequestPayload? = nil, excluding excludedPanelIDs: Set<String> = []) -> [ComposerPanelPayload] {
    ...
    func isCellMergeable(_ cell: ComposerGridCell) -> Bool {
        regionCovering(cell: cell) == nil &&
            occupantPanelID(at: .cell(cell), in: project, excluding: []) == nil
    }

    func nextAvailableID(prefix: String, existing: [String]) -> String {
        var candidate = 1
    ...
    func syncPanelZIndices(in project: inout ComposerRequestPayload) {
        for index in project.panels.indices {
            project.panels[index].zIndex = index
        }
    }

    func hidePanels(withIDs panelIDs: Set<String>, in project: inout ComposerRequestPayload) {
    ...
    func hidePanel(at index: Int, in project: inout ComposerRequestPayload) {
    ...
    func removePanelFromBoard(_ panelID: String) {
        guard let panel = panelByID(panelID), !panel.locked else {
            return
        }
    ...
    func firstAvailableGraphSelection(
        for panel: ComposerPanelPayload,
        in project: ComposerRequestPayload,
    ...
    func firstAvailableAssetTarget(
        in project: ComposerRequestPayload,
        occupiedCells: Set<ComposerGridCell>,
    ...
    func reflowVisiblePanels(in project: inout ComposerRequestPayload) -> Bool {
        ...
        for panelID in visiblePanelIDs {
            ...
            if sourcePanel.kind == "graph" {
                ...
                applyPlacement(
                    panelID: panelID,
                    to: selection.cellCount == 1
                        ? .cell(selection.origin)
                        : .graphSpan(origin: selection.origin, colSpan: selection.colSpan, rowSpan: selection.rowSpan),
                    in: &candidate
                )
    ...
    func rewriteCanonicalOrderFromBoard(in project: inout ComposerRequestPayload) {
    ...
    func boardOrderingKey(for panel: ComposerPanelPayload, in project: ComposerRequestPayload) -> ComposerBoardOrderingKey {
    ...
    var mergeSelectedCellsAvailability: ActionAvailability {
        guard let selection = selectedCellSelection else {
    ...
    var unmergeSelectedRegionAvailability: ActionAvailability {
    ...
    var placementAvailability: ActionAvailability {
    ...
    var removeSelectedPanelAvailability: ActionAvailability {
        guard let panel = selectedPanel else {
    ...
    var manualLabelAvailability: ActionAvailability {
