...
Plot preview has two supported layers. The backend renderer remains the export-grade authority: Swift displays high-resolution PNG for bitmap preview and keeps PDF as the exact/export fallback. A Hybrid Native realtime preview path is allowed for admitted curve-family templates (`curve`, `point_line`, `scatter`, `area_curve`, `step_line`, `function_curve`) when it is fed by the same contract/schema/defaults and can fall back to backend preview when unsupported features are active.

The native canvas layer may be either a pure interaction overlay above PNG/PDF or the Hybrid Native curve-family renderer. In both modes it writes back to typed payloads for guides, annotations, shape overlays, analytical layers, axes, legend order, fit overlays, and series style/offset overrides, then lets `/render-preview` or export redraw the authoritative figure. Native preview must not maintain second template/style/palette/theme constants. If preview metadata is unavailable, the overlay may fall back to axes-fraction placement or backend bitmap preview, but it must not invent untracked chart semantics. Overlay affordances should stay quiet: selected text uses a light outline/baseline rather than a circular marker, callout target placement uses small target ticks, and shape resize handles use small rounded squares instead of button-like dots.

The Plot interaction loop is object-id driven. `PreviewSceneResponse.objects[]` supplies stable object ids, bbox/points, payload refs, visible/locked state, and operation names for series, axes, legend, guides, annotations, shapes, function layers, and fit overlays. Selection and hit-testing route through those ids into the existing module-local inspector categories. Do not add a LabPlot-style global Project Explorer or a shared module rail to expose this object tree.

Every durable Plot edit must be backed by the typed command path: build a local command envelope, call `POST /command/normalize`, call `POST /command/apply-preview`, then store the normalized before/after payload, graph revision, graph patch, and diagnostics in the Plot command ledger. Native `UndoManager` restores the real render/fit snapshot and records a reversible command; it must not be only a visual undo stack.
