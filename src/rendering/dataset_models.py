...
DataShape: TypeAlias = Literal["curve_like", "replicate_table", "matrix", "distribution", "grouped"]
RoleKey: TypeAlias = str
...
def _dataframe_sample_rows(frame: pd.DataFrame, *, limit: int = 6) -> list[list[object]]:
    ...
    for row in frame.head(limit).itertuples(index=False, name=None):
        rows.append([_string_or_none(value) if isinstance(value, str) else _numeric_or_none(value) if isinstance(value, (float, int)) else _clean_text(value) or None for value in row])
    return rows
...
def build_normalized_dataset(
    ...
    *,
    ...
    if resolved_model in {"frequency_sweep", "temperature_sweep", "stress_relaxation"}:
        ...
        first_series_list = next(iter(series_map.values()), [])
    ...
    if resolved_model == "curve_table":
        data_shapes = _data_shapes_for_model("curve_table")
    else:
        data_shapes = _data_shapes_for_model(resolved_model)
