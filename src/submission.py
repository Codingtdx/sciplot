...
from src.composer_types import ComposerProject
...
def _looks_like_pdf(path_text: str) -> bool:
    return Path(path_text).suffix.lower() in _PDF_SUFFIXES


def _looks_like_raster(path_text: str) -> bool:
    return Path(path_text).suffix.lower() in _RASTER_SUFFIXES


def build_composer_submission_report(
    *,
    ...
    visible_panels = [panel for panel in project.panels if not panel.hidden]
    vector_count = sum(_looks_like_pdf(panel.file_path) for panel in visible_panels)
    raster_count = sum(_looks_like_raster(panel.file_path) for panel in visible_panels)
    checks: list[SubmissionCheck] = [
        SubmissionCheck(
            id="vector_raster_mix",
            status="pass" if raster_count == 0 else "advisory",
            message=(
        ...
        SubmissionCheck(
            id="ocg_layer_naming",
            status="pass",
            message="Composer export keeps visible graph, asset, and text drawables in stable OCG layer names.",
            source="export",
        ),
    ]
    ...
    checks.append(
        _qa_issue_check(
            ...
            issue_ids={"micro_text"},
            pass_message="Visible Composer text stays above the editorial minimum size.",
            pending_message="Visible Composer text sizes finalize after preview runs.",
        )
    )
    checks.append(
        _qa_issue_check(
            check_id="canvas_bounds",
            qa_reports=(qa_report,),
            issue_ids={"drawable_outside_canvas", "text_outside_canvas"},
            pass_message="Visible Composer objects stay inside the export canvas.",
            pending_message="Canvas-bound checks finalize after preview runs.",
        )
    )
    checks.append(
        _qa_issue_check(
            check_id="binding_bounds",
            qa_reports=(qa_report,),
            issue_ids={"bound_drawable_overflow", "bound_text_overflow"},
            pass_message="Bound drawables stay inside their assigned regions or slots.",
            pending_message="Binding checks finalize after preview runs.",
        )
    )
    checks.append(
        _qa_issue_check(
            check_id="text_collision",
            qa_reports=(qa_report,),
            issue_ids={"text_collision"},
            pass_message="Composer text boxes are separated cleanly.",
            pending_message="Text-collision checks finalize after preview runs.",
        )
    )

    blockers = (validation_error,) if not valid and validation_error else ()
    readiness = _report_readiness(checks, blockers)
    return SubmissionReport(
        context="composer",
        readiness=readiness,
        summary=_report_summary("composer review", readiness, blockers, checks),
        template="composer",
        output_count=1,
